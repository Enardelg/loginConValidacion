...
function renderizaBotonCerrarSesion() {
    const divUSer = document.querySelector('.user')
    
    buttonJS.innerText = "Cerrar sesion"
    buttonJS.style.padding = "5px 20px"
    buttonJS.style.backgroundColor = "rgba(255,0,0,0.2)"
    buttonJS.style.color = "red"
    buttonJS.style.margin = "20px"
    buttonJS.style.border = "none"
    buttonJS.style.cursor = "pointer"
    divUSer.appendChild(buttonJS)
    

}
renderizaBotonCerrarSesion()
buttonJS.addEventListener('click',() =>{
    let cerrarSesion = confirm('¿Seguro desea cerrar sesión?')
    if(cerrarSesion){
    localStorage.clear()
